...
from pyberry.core.security cimport validate_request
...
async def app(scope, proto):
    ...
    if scope.proto == 'http':
        ...
        # ---------------------------------------------------------
        sec_status = validate_request(scope, config.cors_enabled, config.allowed_hosts)
        if sec_status != 0:
            if sec_status == 400:
                proto.response_str(
                    status=400, 
        ...
        if handler == NULL:
            ...
            if py_handler is not None:
                ...
                injected_kwargs = {}
                for name, p_type in param_meta.items():
                    if name in kwargs:
                        val = kwargs[name]
