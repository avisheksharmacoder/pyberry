...
cpdef object parse_model(object p_type, dict schema, bytes data):
    ...
    while key != NULL:
        ...
        if py_key in schema:
            ...
            setattr(instance, py_key, py_val)
            
        key = yyjson_obj_iter_next(&obj_iter)
        
    for schema_key, meta in schema.items():
        is_required = meta[2]
        if is_required:
            ...
            if meta[1] is not None and not hasattr(instance, schema_key):
                setattr(instance, schema_key, meta[1])
